...
- Claude Code plugins are a first-class `plugin` entry kind (claude adapter): `.claude-plugin/plugin.json` and marketplace manifests are detected by `detectPluginGroups`, installs land in `<scope>/.agentpm/plugins/<name>`, and `syncPluginMarketplace` maintains `.agentpm/plugins/.claude-plugin/marketplace.json` so Claude Code can add the folder as a native marketplace (see `docs/plugin-guide.md`).
